...
test('historySettings setup streams - create stream only if someone listen or emit it', () => {
  const bus = getInstance(new Map([
    ['event:type_2', 1],
    ['event:type_1', 1000]
  ]))

  expect(bus._streams.size).toBe(0)
})
...
test('expect emit event when new stream created with history', () => {
  const subscriber = jest.fn()
